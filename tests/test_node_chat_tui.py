...
def test_python_chat_fallback_accepts_openai_v1_server_base_url(
    fake_openai_server, monkeypatch, capsys
):
    base_url, requests, paths = fake_openai_server
    lines = iter(["hello", "/exit"])
    ...
    assert chat._python_fallback_launch(["--server", f"{base_url}/v1"]) == 0
...
def test_bundled_chat_rejects_empty_model_values(args):
    ...
    assert completed.returncode == 1
    assert "utopic chat: expected a value after -m/--model" in completed.stderr
...
def test_bundled_chat_replaces_stale_partial_model_directory(tmp_path):
    node = shutil.which("node")
    if node is None:
        pytest.skip("node is not installed")
    ...
    models_dir = tmp_path / "models"
    partial_dir = models_dir / "http-model.gguf.partial"
...
def test_bundled_chat_replaces_stale_cached_model_directory(tmp_path):
    node = shutil.which("node")
    if node is None:
        pytest.skip("node is not installed")
    ...
    models_dir = tmp_path / "models"
    model_dir = models_dir / "http-model.gguf"
    ...
    assert completed.returncode == 0, completed.stderr
    assert model_dir.read_bytes() == ModelDownloadServer.body
    assert not (models_dir / "http-model.gguf.partial").exists()
...
def test_bundled_chat_accepts_uppercase_gguf_local_filename(tmp_path):
    node = shutil.which("node")
    if node is None:
        pytest.skip("node is not installed")
