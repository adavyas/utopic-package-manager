...
are cataloged as planned native-implementation surfaces: they expose stable model
...
| Qwen-Image | image | planned native implementation | Cataloged behind `/v1/images/generations` and MCP image generation |
| FLUX.1-schnell | image | planned native implementation | Cataloged behind `/v1/images/generations` and MCP image generation |
| Krea 2 Raw | image | planned native implementation | Cataloged behind `/v1/images/generations` and MCP image generation |
| Cosmos3 Super Text2Image | image | planned native implementation | Cataloged with GPU-memory preflight before native execution |
| Kokoro / Chatterbox / Dia | tts | planned native implementation | Cataloged behind `/v1/audio/speech` and MCP speech generation |
| ACE-Step | music | planned native implementation | Cataloged behind `/v1/audio/generations` and MCP music generation |
| Wan2.1 T2V / LTX-Video | video | planned native implementation | Cataloged behind `/v1/videos/generations` and MCP video generation |
