...
// This is intentionally one-shot: the control plane owns orchestration/debuggability,
// while this binary owns native model loading and generation.
//
...
static json error_response(const runner_error & error) {
    return error_response(runner_error_code_json_name(error.code), error.message, error.detail);
}
...
static const char * runner_task_json_name(runner_task task) {
    switch (task) {
...
int main(int argc, char ** argv) {
    ...
    runner_request req;
    runner_error request_error;
    if (!runner_request_parse_json(root, req, request_error)) {
        json response = error_response(request_error);
        printf("%s\n", response.dump().c_str());
        return 2;
    }
    req.runner = runner_name;
