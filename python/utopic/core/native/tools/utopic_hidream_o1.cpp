...
int main(int argc, char** argv) {
    ...
    if (native_preview) {
        ...
        std::printf("utopic_hidream_o1 native_preview=OK wrote=%s width=%d height=%d steps=%d text_tokens=%lld image_tokens=%lld total_tokens=%lld text_transformer_layers=%d text_transformer_values=%lld text_transformer_l2=%.8f text_transformer_checksum=%.8f image_transformer_layers=%d image_transformer_values=%lld image_transformer_l2=%.8f image_transformer_checksum=%.8f conditioning_values=%lld conditioning_l2=%.8f conditioning_checksum=%.8f patch_values=%lld final_patch_l2=%.8f final_patch_checksum=%.8f backend=native-combined-sequence-pixeldit-no-torch-no-sdcpp\n",
                    ...
                    image_summary.image_transformer_layers,
                    static_cast<long long>(image_summary.image_transformer_values),
                    image_summary.image_transformer_l2,
                    image_summary.image_transformer_checksum,
